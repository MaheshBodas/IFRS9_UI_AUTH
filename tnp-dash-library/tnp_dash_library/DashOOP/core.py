...
class DashApp(DashComponentBase):
    ...
    def _get_dash_app(self):
        ...
        if self.mode == 'dash':
            ...
            app = enrich.DashProxy(transforms=[
                enrich.MultiplexerTransform(),  # makes it possible to target an output multiple times in callbacks
                enrich.ServersideOutputTransform(),  # enable use of ServersideOutput objects
                enrich.NoOutputTransform(),  # enable callbacks without output
            ],
                **self.kwargs
            )
