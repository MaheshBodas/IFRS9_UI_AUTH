...
from flask import Flask, request, render_template, flash, abort, session, redirect, make_response, redirect, url_for, \
    app
...
class FlaskLoginAuth():
    def __init__(self, app, url_base_pathname, tnp_app_id, app_name, tab_name, use_default_views=False, users=None,
                 auto_hash=True,
                 hash_function=None, test_role="super"):
        """
        ...
        """
        self.initial_app = app.app
        self.server = self.initial_app.server
        self.apps_list = [self.initial_app]
        self.url_base_pathname = url_base_pathname
        self.test_role = test_role
        self.tnp_app_id = tnp_app_id
        ...
        if use_default_views:
            ...
            if not users:
                ...
                self.users = UserMap(
                    [DefaultUser('admin', 'admin', 'TNP', self.test_role, self.tnp_app_id, True, self.hash_function)])
                ...
                self.users = UserMap(
                    [DefaultUser(result_set[i][0], result_set[i][2], result_set[i][4], result_set[i][3],
                                 result_set[i][1], self.auto_hash, self.hash_function) for i in
    ...
    def __protect_views(self):
        ...
        for view_func in self.server.view_functions:
            if view_func.startswith(self.url_base_pathname) and not "dash" in view_func:
    ...
    def __default_login_view(self):
        if request.method == 'POST':
            ...
            if user:
                if self.tnp_app_id in user.apps:

                    if password == user.password:

                        hostname = socket.gethostname()
                        ip_address = socket.gethostbyname(hostname)
                        task = (ip_address, hostname, username)
                        task2 = (ip_address, hostname)

                        # make sure only one instance
                        self.__delete_current_user_from_db(task2)

                        # add into logged on to pass information to app
                        self.add_current_user_to_db(task)
                        ...
                        return self.__render_template_dynamic('default_login_dynamic.html')
                else:
                    flash('Login Failed!  You are not authorised to use this TNP application.')
                    return self.__render_template_dynamic('default_login_dynamic.html')
            else:
                flash('Login Failed!  Please try again.')
                return self.__render_template_dynamic('default_login_dynamic.html')
    ...
    def add_current_user_to_db(self, task):
        sql = ''' INSERT INTO LOGGED_ON(IP_ADDRESS,HOSTNAME,USERNAME)
                                     VALUES(?,?,?) '''
        conn = sqlite3.connect(self.sqlite_connection_string)
        cur = conn.cursor()
    ...
    def __default_logout_view(self):
        ...
        return self.__render_template_dynamic('default_logout_dynamic.html')

    def __render_template_dynamic(self, template):

        values = randint(0, 10000, 10)
        val = values[5]

        f = open(os.path.join(TEMPLATE_FOLDER, template), "r")
        template = template.replace('.html', self.tnp_app_id + str(val) + '.html')
        file = os.path.join(TEMPLATE_FOLDER, template)

        copy = open(file, "w")
        for line in f:
            copy.write(line.replace('<APP_ID>', str(self.tnp_app_id).lower())
                       .replace('<APPLICATION_NAME>', str(self.app_name).upper())
                       .replace("<TAB TITLE>", self.tab_name))
        f.close()
        copy.close()

        a = render_template(template)
        os.remove(file)
...
class DefaultUser(UserMixin):
    ...
    def __init__(self, name, organisation, role, apps, password=None, auto_hash=True, hash_function=None):
        self.id = name.lower()
