...
class PiTPDScenarioContent(TNPContent):

    def __init__(self, debug: bool = False):
        EXPANDABLE = True
        ID = 'pd-pit-scenario-container'
        NAME = "Point in Time Probability of Default by Scenario"
    ...
    def content_layout(self, params=None):
        ...
        drop_down = TNPDropdown('pit-scenario', 'Scenario', 'pitscen001', None, LabelPosition.LEFT, 3,
                                None, self.debug, 0, "Select Scenario")
        chart = dcc.Loading(html.Div(id='pd-pit-scenario-chart-child-container'))

        return dcc.Loading(html.Div([drop_down.layout(), chart], id='pd-pit-scenario-chart-container',
                                    hidden=True))
    ...
    def content_call_back(self, app):
        ...
        @app.callback(Output('pd-pit-scenario-chart-child-container', 'children'),
                      Output('pd-pit-scenario-container-modal-body', 'children'),
                      Output('pd-pit-scenario-chart-container', 'hidden'),
                      Input('submit-model', 'n_clicks'),
                      State('model_data', 'data'),
                      State({'type': 'model-dropdown', 'index': 0}, 'value'),
                      State({'type': 'pit-scenario', 'index': 0}, 'value'))
        def update_transition_matrix(n_clicks, model_data, model, scenario):
            if n_clicks is None:
                raise PreventUpdate
            if n_clicks == 0:
                raise PreventUpdate
            chosen_scenario = scenario['value']
            dist, dist2 = get_PiT_PD_chart_data(model, model_data, chosen_scenario)
            return dist2, dist, False

        @app.callback(
            Output('pd-pit-scenario-chart-child-container', 'children'),
            Output('pd-pit-scenario-container-modal-body', 'children'),
            Output('pd-pit-scenario-chart-container', 'hidden'),
        ...
        @app.callback(
            ...
            Output('pd-pit-scenario-container-modal-body', 'children'),
            Output('pd-pit-scenario-chart-container', 'hidden'),
            Input({'type': 'pit-scenario', 'index': 0}, 'value'),
            State('model_data', 'data'),
            State({'type': 'model-dropdown', 'index': 0}, 'value'),
            State('pd-transition-matrix-chart-container', 'hidden'),
            prevent_initial_call = True)
        def update_transition_matrix(scenario, model_data, model, hidden):
            if hidden:
                raise PreventUpdate
        ...
        @app.callback(Output({'type': 'pit-scenario', 'index': 0}, 'options'),
                      Output({'type': 'pit-scenario', 'index': 0}, 'value'),
                      Input({'type': 'model-dropdown', 'index': 0}, 'value'),
                      State('model_data', 'data'))
        def update_drop_down_values(model, model_data):
            ...
            scenarios = json.loads(uz.json_unzip(model_data)['pd_data'])[model]['scenarioName']
...
def get_PiT_PD_chart_data(model, model_data, scenario):
    uncompressed_data = json.loads(uz.json_unzip(model_data)['pd_data'])[model]
    fig1 = pdc.plotPDCurveAllGrades(uncompressed_data, scenario)
