...
class PiTPDContent(TNPContent):

    def __init__(self, debug: bool = False):
        EXPANDABLE = True
        ID = 'pd-pit-grade-container'
        NAME = "Point in Time Probability of Default by Grade"
    ...
    def content_layout(self, params=None):
        ...
        drop_down = TNPDropdown('pit-grade', 'Grade', 'pit000', None, LabelPosition.LEFT, 3,
                                None, self.debug, 0, "Select Model Grade")
        chart = dcc.Loading(html.Div(id='pd-pit-grade-chart-child-container'))

        return dcc.Loading(html.Div([drop_down.layout(), chart], id='pd-pit-grade-chart-container',
                                    hidden=True))
    ...
    def content_call_back(self, app):
        ...
        @app.callback(Output('pd-pit-grade-chart-child-container', 'children'),
                      Output('pd-pit-grade-container-modal-body', 'children'),
                      Output('pd-pit-grade-chart-container', 'hidden'),
                      Input('submit-model', 'n_clicks'),
                      State('model_data', 'data'),
                      State({'type': 'model-dropdown', 'index': 0}, 'value'),
                      State({'type': 'pit-grade', 'index': 0}, 'value'))
        ...
        @app.callback(
            Output('pd-pit-grade-chart-child-container', 'children'),
            Output('pd-pit-grade-container-modal-body', 'children'),
            Output('pd-pit-grade-chart-container', 'hidden'),
        ...
        @app.callback(
            ...
            Output('pd-pit-grade-container-modal-body', 'children'),
            Output('pd-pit-grade-chart-container', 'hidden'),
            Input({'type': 'pit-grade', 'index': 0}, 'value'),
            State('model_data', 'data'),
            State({'type': 'model-dropdown', 'index': 0}, 'value'),
        ...
        @app.callback(Output({'type': 'pit-grade', 'index': 0}, 'options'),
                      Output({'type': 'pit-grade', 'index': 0}, 'value'),
                      Input({'type': 'model-dropdown', 'index': 0}, 'value'),
                      State('model_data', 'data'))
...
def get_PiT_PD_chart_data(model, model_data, grade):
    ...
    fig1 = pdc.plotPDCurveAllScenarios(uncompressed_data, grade)
