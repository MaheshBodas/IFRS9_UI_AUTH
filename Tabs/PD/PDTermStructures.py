...
class PDTermStructureContent(TNPContent):

    def __init__(self, debug: bool = False):
        EXPANDABLE = True
        ID = 'ts-container'
        NAME = "Probability of Default Term Structures"
    ...
    def content_layout(self, params=None):
        ...
        chart = dcc.Loading(html.Div(id='ts-chart-child-container'))

        return dcc.Loading(html.Div([chart], id='ts-chart-container',
                                    hidden=True))
    ...
    def content_call_back(self, app):
        ...
        @app.callback(Output('ts-chart-child-container', 'children'),
                      Output('ts-container-modal-body', 'children'),
                      Output('ts-chart-container', 'hidden'),
        ...
        def update_transition_matrix(n_clicks, model_data, model):
            ...
            if n_clicks == 0:
                raise PreventUpdate
            dist, dist2 = get_ts(model, model_data)
            return dist2, dist, False

        @app.callback(
            Output('ts-chart-child-container', 'children'),
            Output('ts-container-modal-body', 'children'),
            Output('ts-chart-container', 'hidden'),
...
def get_ts(model, model_data):
    uncompressed_data = json.loads(uz.json_unzip(model_data)['pd_data'])[model]
    fig1 = pdc.plotTermStructures(uncompressed_data)
